...
public class TaggedBufferBeforeSpliterators<T> {
    ...
    public TaggedBufferBeforeSpliterators(Predicate<? super T> fun) {
    ...
    public List<T> asTaggedList() {
        ...
        class TaggedList extends AbstractList<T> implements RandomAccess {
            ...
            public T get(int index) {
                return TaggedBufferBeforeSpliterators.this.elements[filteredIndexes[index]];
            }
    ...
    public Iterator<T> iterator(boolean onlyTagged) {
        return new Iterator<T>() {
            ...
            private final List<T> eltVue = Arrays.asList(TaggedBufferBeforeSpliterators.this.elements);
            private final int vueEltSize = TaggedBufferBeforeSpliterators.this.eltSize;
            private final int vueFilteredEltSize = TaggedBufferBeforeSpliterators.this.filteredEltSize;
            ...
            public T next() {
                ...
                if (onlyTagged) {
                    ...
                    if (!TaggedBufferBeforeSpliterators.this.predicate.test(elt)){
                        index++;
                        return next();
                    } else {
                        filteredIndex++;
    ...
    public Stream<T> stream(boolean onlyTagged) {
        T[] array;
        if(onlyTagged){
            array = (T[]) asTaggedList().toArray();
        } else {
            array = (T[]) Arrays.stream(this.elements)
                    .filter(Objects::nonNull)
                    .toArray();
        }
        return StreamSupport.stream(createSpliterator(0, array.length, array), true);
    }
